...
public class Browser {
	...
	private void promptUser() {
		System.out.println("\nBrowser ready.");
		System.out.println("Commands: visit <url>, history, back, forward, quit");
	}
	...
	private void interpretAndExecute(String cmd) {
		...
		if (cmd.equals("back")) {
			...
			if (commands.length > 1) {
				...
				System.out.println("Please enter URL: visit google.com");
			}
		} else if (cmd.equals("quit")) {
			quit();
		} else if (cmd.equals("forward")) {
	...
	private void goBack() {
		if (currentVisit != null && currentVisit.getPreviousNode() != null) {
			System.out.println("Going back...");
			currentVisit = currentVisit.getPreviousNode();
			System.out.println(currentVisit);
		} else {
